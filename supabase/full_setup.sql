-- ============================================================
-- GraphSentinel: FULL SCHEMA + SEED (consolidated)
...
-- â”€â”€ pgvector extension â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
CREATE EXTENSION IF NOT EXISTS vector;

-- â”€â”€ alert_embeddings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ graph_communities (FalkorDB-style) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ graph_paths (shortest-path / layering chains) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/*
  ...
  1. Multi-hop Layering (accounts A01 â†’ A05 chain)
  2. Circular Round-Trip (accounts B01 â†’ B04 â†’ B01)
  3. Structuring below â‚¹10L threshold (account C01 repeated transfers)
  4. Dormant Account Reactivation (account D01, inactive 8 months, high-value transfer)
  5. KYC Mismatch (account E01, declared income â‚¹3L, transacts â‚¹45L/month)
*/
...
('pat_structuring', 'Structuring / Smurfing', 'Multiple transactions just below the â‚¹10 Lakh reporting threshold within a rolling time window', 999999, 72, 1, 1.0, true),
...
-- Multi-hop layering chain (A01 â†’ A02 â†’ A03 â†’ A04 â†’ A05 â†’ A06)
...
-- Circular round-trip (B01 â†’ B02 â†’ B03 â†’ B04 â†’ B01)
...
(
  ...
  7826000,
  0.89,
  'ACC_C01 (Ramesh Narayan Patil, Pune Deccan) executed 8 transactions over 72 hours, each precisely between â‚¹9,60,000 and â‚¹9,90,000 â€” all below the â‚¹10,00,000 mandatory reporting threshold. The aggregate value of â‚¹78,26,000 would trigger both STR and CTR obligations if transacted as a single transfer. The four recipient accounts (C02â€“C05) are registered businesses in the same geographic cluster (Pune), and all received funds have been partially forwarded to clean accounts within 24 hours. This pattern matches known smurfing behaviour with 89% model confidence. Key SHAP drivers: sub-threshold amount clustering (52%), high transaction frequency in 72-hour window (31%), recipient account forwarding behaviour (17%).',
  '[{"factor": "Sub-threshold amount clustering â‚¹9.6L-â‚¹9.9L", "weight": 0.52, "direction": "increases_risk"}, {"factor": "8 transactions in 72-hour window", "weight": 0.31, "direction": "increases_risk"}, {"factor": "Recipient accounts forward funds within 24h", "weight": 0.17, "direction": "increases_risk"}]',
  'high',
  'open',
...
(
  ...
  7500000,
  0.87,
  'ACC_D01 (Sudhir Malhotra, Kolkata Salt Lake) was dormant for 8 months and 12 days with zero transaction activity. Within 2 hours of reactivation, the account initiated a â‚¹7,50,00,000 RTGS transfer to Eastern Traders Consortium (ACC_D02), which immediately forwarded â‚¹7,20,00,000 to Howrah Bridge Investments (ACC_D03). The account holder is a retired individual (declared income: â‚¹3,00,000/year), making this single transfer 250x the declared annual income. The rapid reactivation-and-forward pattern is strongly consistent with the account being used as a pass-through for third-party funds. SHAP analysis: dormancy duration 8 months (41%), transfer amount vs declared income ratio 250x (35%), immediate forwarding chain (24%).',
...
(
  ...
  880000,
  0.78,
  'A second circular round-trip pattern between the same 4 entities (B01â€“B04) was identified 4 days before the primary round-trip. The smaller transaction size (â‚¹9.5L origin, â‚¹8.8L returned â€” 92.6% recovery) and the same set of accounts confirm a recurring money circulation operation. The consistency of the 92% recovery ratio across both rounds suggests a systematic fee arrangement for the circulation service. SHAP: same entity set as prior alert (48%), consistent 92% recovery ratio (32%), NEFT channel vs RTGS in primary alert (20%).',

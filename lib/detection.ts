...
import type { Account, FraudAlert, FraudPattern, GraphData, GraphEdge, ShapFactor, Transaction, DashboardSummary } from './types';
...
function severityFromConfidence(confidence: number, totalAmount: number) {
  if (confidence >= 0.9 || totalAmount >= 50000000) return 'critical';
  if (confidence >= 0.78 || totalAmount >= 10000000) return 'high';
  if (confidence >= 0.62 || totalAmount >= 1000000) return 'medium';
  return 'low';
}

function makeAlert({
  patternType, accounts, transactions, totalAmount, confidenceScore, shapNarrative, shapFactors, notes = '',
}: {
  patternType: string;
  accounts: string[];
...
function accountLabel(accountMap: Map<string, Account>, accountId: string) {
  return accountMap.get(accountId)?.holder_name || accountId;
}

function accountAgeDays(account: Account) {
  return daysBetween(account.created_at, new Date().toISOString());
}

function recentDormancyDays(account: Account) {
  return daysBetween(account.last_activity_at, new Date().toISOString());
}

function avgForwardRatio(path: Transaction[]) {
...
function buildAdjacency(transactions: Transaction[]) {
  const adjacency = groupBy(transactions, (transaction) => transaction.sender_account_id);

  for (const edges of adjacency.values()) {
    edges.sort((left, right) => new Date(left.timestamp).getTime() - new Date(right.timestamp).getTime());
  }

  return adjacency;
}

function windowTransactions(transactions: Transaction[], windowHours: number) {
...
function detectMultiHopLayering({ accounts, transactions, pattern }: { accounts: Account[]; transactions: Transaction[]; pattern: Partial<FraudPattern> }) {
  ...
  function walk(path: Transaction[]) {
    ...
    for (const nextTransaction of nextTransactions) {
      ...
      if (new Date(nextTransaction.timestamp).getTime() < new Date(lastTransaction.timestamp).getTime()) {
        continue;
      }
...
function detectCircularRoundTrip({ accounts, transactions, pattern }: { accounts: Account[]; transactions: Transaction[]; pattern: Partial<FraudPattern> }) {
  const accountMap = new Map(accounts.map((account) => [account.id, account]));
  ...
  function walk(startTransaction: Transaction, path: Transaction[]) {
    const lastTransaction = path[path.length - 1];
    const nextTransactions = adjacency.get(lastTransaction.receiver_account_id) || [];
...
function detectStructuring({ accounts, transactions, pattern }: { accounts: Account[]; transactions: Transaction[]; pattern: Partial<FraudPattern> }) {
  const accountMap = new Map(accounts.map((account) => [account.id, account]));
  const recentTransactions = windowTransactions(transactions, pattern.time_window_hours || 72);
  ...
  for (const [senderId, senderTransactions] of bySender.entries()) {
    const sortedTransactions = [...senderTransactions].sort((left, right) => new Date(left.timestamp).getTime() - new Date(right.timestamp).getTime());
...
function detectDormantReactivation({ accounts, transactions, pattern }: { accounts: Account[]; transactions: Transaction[]; pattern: Partial<FraudPattern> }) {
...
function detectKycMismatch({ accounts, transactions, pattern }: { accounts: Account[]; transactions: Transaction[]; pattern: Partial<FraudPattern> }) {
...
export function detectFraudAlerts({ accounts, transactions, patterns }: { accounts: Account[]; transactions: Transaction[]; patterns: FraudPattern[] }) {
...
export function summarizeGraph(accounts: Account[], transactions: Transaction[], alerts: FraudAlert[]): GraphData {
  ...
  for (const transaction of transactions) {
    ...
    if (new Date(transaction.timestamp).getTime() > new Date(existing.last_transaction_at).getTime()) {
...
export function summarizeDashboard({ accounts, transactions, alerts, reports, nodes }: {
  ...
  transactions: Transaction[];
  alerts: FraudAlert[];
  reports: { submission_status: string }[];
  nodes: { status: string }[];
}): DashboardSummary {
